...
import com.yk.silenct.datepicker.util.LogUtils;
import com.yk.silenct.datepicker.util.MonthUtils;
...
import com.yk.silenct.datepicker.wheelView.wheeladapter.ArrayWheelAdapter;
...
public class DateTimePicker extends WheelPicker {
    ...
    private int startYear = 2010, startMonth = January, startDay = 1;
    private int endYear = 2020, endMonth = December, endDay = 31;
...
//        wheelViewParams.gravity = Gravity.CENTER;
        if (wheelModeEnable) {
            ...
            if (dateMode != NONE) {
                ...
                monthView.setAdapter(new ArrayWheelAdapter<>(MonthUtils.getMonth()));
            ...
            if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
                ...
                yearView.setOnItemPickListener(new OnItemPickListener<String>() {
                    ...
                    public void onItemPicked(int index, String item) {
                        ...
                        selectedMonthIndex = January;//重置月份索引
                        selectedDayIndex = 0;//重置日子索引
                        //需要根据年份及月份动态计算天数
...
//                        LogUtils.verbose(this, "change minutes after hour wheeled");
    ...
    private void changeMonthData(int selectedYear) {
        months.clear();
        if (startMonth < January || endMonth < January || startMonth > December || endMonth > December) {
        ...
        if (startYear == endYear) {
            ...
            for (int i = startMonth; i <= December; i++) {
                months.add(DateUtils.fillZero(i));
            }
        } else if (selectedYear == endYear) {
            for (int i = January; i <= endMonth; i++) {
                months.add(DateUtils.fillZero(i));
            }
        } else {
